...
/-- While in many cases `Fin.castIso` is better than `Equiv.cast`/`cast`, sometimes we want to apply
a generic theorem about `cast`. -/
theorem castIso_eq_castIso (h : n = m) : (castIso h : Fin n → Fin m) = castIso (h ▸ rfl) := by
  subst h
  ext
  simp
#align fin.cast_eq_cast Fin.castIso_eq_castIso
...
@[simp]
theorem castAddEmb_zero : (castAddEmb 0 : Fin n → Fin (n + 0)) = cast rfl := by
  ext
...
/-- For rewriting in the reverse direction, see `Fin.cast_castAddEmb_left`. -/
...
theorem cast_castAddEmb_left {n n' m : ℕ} (i : Fin n') (h : n' + m = n + m) :
    castIso h (castAddEmb m i) = castAddEmb m (castIso (add_right_cancel h) i) := by
  ext
  simp
#align fin.cast_cast_add_left Fin.cast_castAddEmb_left

@[simp]
theorem cast_castAddEmb_right {n m m' : ℕ} (i : Fin n) (h : n + m' = n + m) :
    castIso h (castAddEmb m' i) = castAddEmb m i := by
  ext
  simp
#align fin.cast_cast_add_right Fin.cast_castAddEmb_right
...
@[simp]
theorem castLT_castSuccEmb {n : ℕ} (a : Fin n) (h : (a : ℕ) < n) : castLT (castSuccEmb a) h = a := by
  cases a; rfl
#align fin.cast_lt_cast_succ Fin.castLT_castSuccEmb

--@[simp] Porting note: simp can prove it
theorem castSuccEmb_lt_castSuccEmb_iff {a b : Fin n} : Fin.castSuccEmb a < Fin.castSuccEmb b ↔ a < b :=
...
theorem castSuccEmb_fin_succ (n : ℕ) (j : Fin n) : castSuccEmb (Fin.succ j) = Fin.succ (castSuccEmb j) := by
  simp [Fin.ext_iff]
...
@[simp]
theorem cast_addNatEmb_zero {n n' : ℕ} (i : Fin n) (h : n + 0 = n') :
    cast h (addNatEmb 0 i) = cast ((add_zero _).symm.trans h) i :=
  ext <| add_zero _
#align fin.cast_add_nat_zero Fin.cast_addNatEmb_zero

/-- For rewriting in the reverse direction, see `Fin.cast_addNatEmb_left`. -/
theorem addNatEmb_cast {n n' m : ℕ} (i : Fin n') (h : n' = n) :
    addNatEmb m (cast h i) = cast (congr_arg (. + m) h) (addNatEmb m i) :=
  ext rfl
#align fin.add_nat_cast Fin.addNatEmb_cast

theorem cast_addNatEmb_left {n n' m : ℕ} (i : Fin n') (h : n' + m = n + m) :
    cast h (addNatEmb m i) = addNatEmb m (cast (add_right_cancel h) i) := by
  ext
  simp
#align fin.cast_add_nat_left Fin.cast_addNatEmb_left

@[simp]
theorem cast_addNatEmb_right {n m m' : ℕ} (i : Fin n) (h : n + m' = n + m) :
    cast h (addNatEmb m' i) = addNatEmb m i :=
  ext <| (congr_arg ((· + ·) (i : ℕ)) (add_left_cancel h) : _)
#align fin.cast_add_nat_right Fin.cast_addNatEmb_right
...
@[simp]
theorem cast_natAdd (n : ℕ) {m : ℕ} (i : Fin m) : cast (add_comm _ _) (natAdd n i) = addNatEmb n i :=
  ext <| add_comm _ _
#align fin.cast_nat_add Fin.cast_natAdd

@[simp]
theorem cast_addNatEmb {n : ℕ} (m : ℕ) (i : Fin n) : cast (add_comm _ _) (addNatEmb m i) = natAdd m i :=
  ext <| add_comm _ _
#align fin.cast_add_nat Fin.cast_addNatEmb
...
theorem natAdd_castSuccEmb {m n : ℕ} {i : Fin m} : natAdd n (castSuccEmb i) = castSuccEmb (natAdd n i) :=
  rfl
...
theorem natAdd_subNat_cast {i : Fin (n + m)} (h : n ≤ i) :
    natAdd n (subNat n (cast (add_comm _ _) i) h) = i := by simp [← cast_addNatEmb]
...
def predAbove (p : Fin n) (i : Fin (n + 1)) : Fin n :=
  if h : castSuccEmb p < i then i.pred (_root_.ne_of_lt (lt_of_le_of_lt (zero_le (castSuccEmb p)) h)).symm
...
theorem castSuccEmb_pred_eq_pred_castSuccEmb {a : Fin (n + 1)} (ha : a ≠ 0)
    (ha' := a.castSuccEmb_ne_zero_iff.mpr ha) : castSuccEmb (a.pred ha) = (castSuccEmb a).pred ha' := by
  cases a
  rfl
  ...
  · cases' n with n
    ...
    · rw [Fin.predAbove_below a b h₂,
        Fin.predAbove_below a.succ b.succ
          (by
            simpa only [le_iff_val_le_val, val_succ, coe_castSuccEmb, add_le_add_iff_right] using h₂)]
      ext
      have h₀ : (b : ℕ) < n + 1 := by
...
theorem castSuccEmb_castPred {i : Fin (n + 2)} (h : i < last (n + 1)) : castSuccEmb i.castPred = i := by
