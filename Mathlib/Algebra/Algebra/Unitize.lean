...
variable {S R A : Type _} [SMul R A] [Mul A] [SetLike S A] [SMulMemClass S R A] [MulMemClass S A]

instance NonUnitalSubalgebraClass.instIsScalarTower [IsScalarTower R A A] (s : S) :
    IsScalarTower R s s where
  smul_assoc r x y := Subtype.ext <| smul_assoc r (x : A) (y : A)

instance NonUnitalSubalgebraClass.instSMulCommClass [SMulCommClass R A A] (s : S) :
    SMulCommClass R s s where
