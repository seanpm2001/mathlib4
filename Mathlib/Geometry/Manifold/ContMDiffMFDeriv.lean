    ...
    ContDiffOn 𝕜 m
      (((fun p : H' × E' => (I' p.fst, p.snd)) ∘ Equiv.sigmaEquivProd H' E') ∘
        tangentMapWithin I I' f s ∘
          (Equiv.sigmaEquivProd H E).symm ∘ fun p : E × E => (I.symm p.fst, p.snd))
      ((range I ∩ I.symm ⁻¹' s) ×ˢ univ)
  · simp only [extChartAt, LocalHomeomorph.extend]
    simp only [tangentBundle_model_space_chartAt]
    simp only [modelWithCorners_prod_toLocalEquiv]
    simp only [modelWithCornersSelf_localEquiv]
    simp only [LocalEquiv.coe_trans]
    simp only [LocalEquiv.prod_coe]
    simp only [ModelWithCorners.toLocalEquiv_coe]
    simp only [LocalEquiv.refl_coe]
    simp only [id.def]
    simp only [Equiv.toLocalEquiv_apply]
    simp only [LocalEquiv.coe_trans_symm]
    simp only [Equiv.toLocalEquiv_symm_apply]
    simp only [LocalEquiv.prod_symm]
    simp only [LocalEquiv.refl_symm]
    simp only [LocalEquiv.prod_coe]
    simp only [ModelWithCorners.toLocalEquiv_coe_symm]
    simp only [LocalEquiv.trans_target]
    simp only [LocalEquiv.prod_target]
    simp only [ModelWithCorners.target_eq]
    simp only [LocalEquiv.refl_target]
    simp only [Equiv.toLocalEquiv_target]
    simp only [preimage_univ]
    simp only [inter_univ]
    simp only [LocalEquiv.trans_source]
    simp only [Equiv.toLocalEquiv_source]
    simp only [LocalEquiv.prod_source]
    simp only [ModelWithCorners.source_eq]
    simp only [LocalEquiv.refl_source]
...
theorem tangentMap_tangentBundle_pure (p : TangentBundle I M) :
  ...
  have N : I.symm ⁻¹' (chartAt H x).target ∈ 𝓝 (I ((chartAt H x) x)) := by
    ...
    simp only [mfld_simps]
  have A : MDifferentiableAt I I.tangent (fun x => @totalSpaceMk M (TangentSpace I) x 0) x :=
    haveI : Smooth I (I.prod 𝓘(𝕜, E)) (zeroSection (TangentSpace I : M → Type _)) :=
  ...
  simp only [Bundle.zeroSection, tangentMap, mfderiv, TotalSpace.proj_mk, A, if_pos, chartAt,
